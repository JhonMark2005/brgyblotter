...
<main id="main-content" class="flex-1 overflow-auto bg-gray-50">
        ...
        <div class="flex items-center gap-2">
            ...
            <div>
                <div class="flex items-center gap-1.5 text-sm text-gray-500 mb-1">
                    ...
                    <span class="text-gray-700 font-medium">{{ __('user.edit') }}</span>
                </div>
                <h1 class="text-xl font-semibold text-gray-800">{{ __('user.edit_account') }}</h1>
            </div>
        </div>
    </div>
    ...
    <div class="p-4 md:p-6 max-w-lg mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-gradient-to-r from-amber-500 to-amber-400 px-6 py-5">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-white font-bold text-lg">
                        {{ strtoupper(substr($user['full_name'], 0, 1)) }}
                    </div>
                    <div>
                        <p class="text-white font-semibold text-base">{{ $user['full_name'] }}</p>
                        <p class="text-amber-100 text-xs">@{{ $user['username'] }} &bull; {{ ucfirst($user['role']) }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        <label for="password" class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
                            {{ __('user.new_password') }} <span class="text-gray-400 font-normal normal-case">({{ __('user.password_note') }})</span>
                        </label>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <label class="relative cursor-pointer">
                                <input type="radio" name="role" value="staff" class="peer sr-only" @checked($user['role'] === 'staff')>
                            ...
                            <label class="relative cursor-pointer">
                                <input type="radio" name="role" value="admin" class="peer sr-only" @checked($user['role'] === 'admin')>
            ...
            @if((int) $user['id'] !== (int) (session('user')['id'] ?? 0))
            <div class="px-6 py-4 border-t border-dashed border-red-100 bg-red-50/30">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-semibold text-red-600">{{ __('user.delete_account') }}</p>
        ...
        <div class="mt-3 bg-white rounded-xl border border-gray-100 px-5 py-3 text-xs text-gray-500">
            <span class="font-semibold text-gray-600">{{ __('user.account_since') }}</span> {{ !empty($user['created_at']) ? date('F j, Y', strtotime($user['created_at'])) : 'N/A' }}
        </div>
    </div>
</main>
@endsection
